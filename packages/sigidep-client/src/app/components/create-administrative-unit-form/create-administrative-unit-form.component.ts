...
export class CreateAdministrativeUnitFormComponent extends BaseComponent implements OnInit {
  ...
  get codeBase(): string {
    return `${this.codeBaseParts.categoryCode ?? 'XX'}${this.codeBaseParts.regionCode ?? 'XX'}`;
  }
  ...
  ngOnInit(): void {
    ...
    forkJoin([
      ...
      this.functions = (array[0] ?? []).map(item => new FunctionModel(item));
    ...
    this.form.get('primaryFunction')
      ...
      ?.subscribe(res => {
        this.secondaryFunction = (res?.children ?? []).map((item: FunctionModel) => new FunctionModel(item));
        if (!res)
          this.form.get('secondaryFunction')?.patchValue(null);
      })
  }
  ...
  submit() {
    ...
    this._apisService.post<AdministrativeUnitModel>('/administrative-units', {
