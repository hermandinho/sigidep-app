...
import {
  ...
  GetAdministrativeUnitesSuccess, GetFunctions, GetFunctionsFailure, GetFunctionsSuccess
} from "@store/actions";
...
export class AdministrativeUnitsEffects {
  fetch$ = createEffect(() =>
    this.actions$.pipe(
      ...
      mergeMap((action) =>
        this.apisService.get<AdministrativeUnitModel[]>('/administrative-units').pipe(
          switchMap((payload) => {
            return [GetAdministrativeUnitesSuccess({payload})];
          }),
          catchError((err: HttpErrorResponse) => of(GetAdministrativeUnitesFailure(err)))
        )
      )
    )
  );
  ...
  delete$ = createEffect(() =>
    this.actions$.pipe(
      ...
      mergeMap((action) =>
        this.apisService.delete<any>(`/administrative-units/${action.id}`, {}).pipe(
          switchMap((payload) => {
            return [DeleteAdministrativeUnitSuccess(), GetAdministrativeUnites()];
          }),
          catchError((err: HttpErrorResponse) => of(GetAdministrativeUnitesFailure(err)))
        )
      )
  ...
  filterFunctions$ = createEffect(() =>
    this.actions$.pipe(
      ofType(GetFunctions),
      mergeMap((action) =>
        this.apisService.get<FunctionModel[]>(`/functions`, { type: action._type }).pipe(
          switchMap((payload) => {
