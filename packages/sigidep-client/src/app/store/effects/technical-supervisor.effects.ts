...
export class TechnicalSupervisorEffects {
  fetch$ = createEffect(() =>
    this.actions$.pipe(
      ...
      mergeMap((action) =>
        this.apisService.get<TechnicalSupervisorModel[]>('/technical-supervisors').pipe(
          switchMap((payload) => {
            return [GetTechnicalSupervisorsSuccess({payload})];
          }),
          catchError((err: HttpErrorResponse) => of(GetTechnicalSupervisorsFailure(err)))
        )
      )
    )
  );
  ...
  delete$ = createEffect(() =>
    this.actions$.pipe(
      ...
      mergeMap((action) =>
        this.apisService.delete<any>(`/technical-supervisors/${action.id}`, {}).pipe(
          switchMap((payload) => {
            return [DeleteTechnicalSupervisorSuccess(), GetTechnicalSupervisors()];
          }),
          catchError((err: HttpErrorResponse) => of(DeleteTechnicalSupervisorFailure(err)))
        )
      )
    )
  );
